...
## General text cleansing for all stocks
sld <- getSLD() %>%
  filter(ActiveYear %in% 2015)

##################
# Data wrangling #
##################

## Stocks are listed as present (x) or absent () for each ecoregions (multiple columns)
ecoregions <- colnames(sld)[grepl("^.+(Ecoregion)$", colnames(sld))]

dots <- lapply(c("StockCode", "Description", "SectionNumber", "AdviceType", ecoregions, "DataCategory"),
               as.symbol)

# Data wrangling to: transform wide data into long data, clean column names, correct ecoregion names, and modify
# ecoregion names according to the 70% rule (if a stock is in 70% of the ecoregions it is Northeast Atlantic).
sl <- sld %>%
  select_(.dots = dots) %>%
  gather(variable, value, -StockCode, -Description, -SectionNumber, -AdviceType, -DataCategory) %>%
  filter(!is.na(value)) %>%
  ...
  mutate(STOCK.CODE = tolower(STOCK.CODE),
         ...
         ECOREGION = recode(ECOREGION, "ArcticOceanEcoregion" = "Arctic Ocean"),
  ...
  unite(ADVICE.HEADING, 7:18, sep = ", ", remove = FALSE) %>%
  mutate(STOCK.NAME = gsub(" $","", STOCK.NAME, perl = TRUE), # remove trailing spaces
         ADVICE.HEADING = gsub("NA, ", "", ADVICE.HEADING),
         ADVICE.HEADING = gsub(", NA", "", ADVICE.HEADING),
         ...
         ADVICE.TYPE = ifelse(ADVICE.TYPE == "MSY/PA",
                              "MSY", ADVICE.TYPE)) %>%
  select(STOCK.CODE, STOCK.NAME, SECTION.NUMBER, ADVICE.TYPE, CATEGORY, ADVICE.HEADING, CAPTION.NAME)


# output <- paste0(sharePoint, "tester.docx")
...
# Category 3
cat3 <- sl[sl$CATEGORY == 3 &
             complete.cases(sl),]

# Category 4
cat4 <- sl[sl$CATEGORY == 4 &
             complete.cases(sl),]

# Category 5
cat5 <- sl[sl$CATEGORY == 5 &
             complete.cases(sl),]

# Category 6
cat6 <- sl[sl$CATEGORY == 6 &
             complete.cases(sl),]
...
stockDat <- cat3 %>%
  ...
  # styles(draftDoc)
  draftDoc <- draftDoc %>%
    # Add ecoregion and release date
